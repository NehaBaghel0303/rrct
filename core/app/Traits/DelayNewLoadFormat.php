...
trait DelayNewLoadFormat
{
    function delayNewLoadFormat($length = 10){
        
        $data = LorryReceipt::query();
        $data->with('remarks',function($q){
            $q->where('user_id',auth()->id())->where('type','delay_eta_at_nl');
        });
        ...
        return view('backend.report.delay-for-new-load.load',compact('data'));
    }
