...
trait DieselFormat
{
    function dieselFormat($length = 10){
        
        $data = LorryReceipt::query();

        $data->with('remarks',function($q){
            $q->where('user_id',auth()->id())->where('type','diesel');
        });
        ...
        foreach($data as $d){
            ...
            $item['tot_dsl_ltr'] = getTotalDieselLiter();
            $item['tot_dsl_amt'] = getTotalFastTagAmount();
            $item['act_dsl_ltr'] = getActualDieselLiter();
            $item['act_dsl_amt'] = getActualFastTagAmount();
            $item['var_dif_amt'] = getVarianceDifferenceAmount();
            $item['remark'] = "Update Remark";
            $d['item'] =  (object)$item;
        }
        return view('backend.report.diesel.load',compact('data'));
    }
