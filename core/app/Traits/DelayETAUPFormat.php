...
trait DelayETAUPFormat
{
    function delayETAUPFormat($length = 10){
       
        $data = LorryReceipt::query();
        $data->with('remarks',function($q){
            $q->where('user_id',auth()->id())->where('type','delay_eta_at_up');
        });
        ...
        return view('backend.report.delay-at-unloading-point.load',compact('data'));
    }
